#!/usr/bin/env python
"""LLM training comparison: LSLinear GPT vs Standard GPT vs INN LM.

Demonstrates LSLinear's parameter efficiency on language modeling by training
three model variants on WikiText and comparing perplexity, throughput, and
memory usage.

Variants:
  1. Standard GPT   — decoder-only transformer with nn.Linear
  2. LSLinear GPT    — identical GPT, LSLinear replaces all linear projections
  3. INN LM          — Iterative Neural Network with Sequential2D + LSLinear

Usage
-----
Quick smoke test (minutes):
    uv run --extra llm examples/llm/llm_comparison.py --config small --epochs 2

Full small comparison:
    uv run --extra llm examples/llm/llm_comparison.py --config small --epochs 20

Large-scale multi-GPU:
    torchrun --nproc_per_node=4 examples/llm/llm_comparison.py \\
        --config large --dataset wikitext-103 --epochs 10

Single model only:
    uv run --extra llm examples/llm/llm_comparison.py --models ls --config medium
"""
...
# Allow imports from this directory (must be first to shadow comparisons/models.py)
...
@click.command()
@click.option("--config", "config_name", type=click.Choice(["small", "medium", "large"]),
              default="small", show_default=True,
              help="Preset model configuration.")
@click.option("--models", multiple=True, type=click.Choice(ALL_MODEL_NAMES),
              default=ALL_MODEL_NAMES, show_default=True,
...
@click.option("--iterations", type=int, default=None, help="Override INN iterations.")
...
def main(config_name, models, dataset, d_model, n_layers, n_heads, d_ff,
         context_len, num_blocks, rank, iterations, batch_size, epochs, lr,
         weight_decay, grad_clip, amp, device, seed, save_json, log, quiet):
    """Train and compare Standard GPT, LSLinear GPT, and INN LM on WikiText."""
    ...
    # Resolve effective context_len for data loading
    effective_cfg = dict(MODEL_CONFIGS[config_name])
    ...
    # -- Header -------------------------------------------------------------
    print(f"\n{'=' * 80}")
    print(f" LLM Comparison -- {dataset} (GPT-2 tokenizer, context={ctx_len})")
    print(f"{'=' * 80}")
    ...
    print(f"  Config      : {config_name} "
          ...
          f"heads={effective_cfg['n_heads']}, ff={effective_cfg['d_ff']})")
